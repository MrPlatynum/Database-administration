...
7. **Анализ снимков транзакций и видимость версий строк.**  
   Используя функцию `pg_current_snapshot()`, проанализированы снимки двух параллельных транзакций (A и B):
...
8. **Поведение функций STABLE и VOLATILE на разных уровнях изоляции.**  
   ...
   - В `REPEATABLE READ` даже функции `VOLATILE` используют фиксированный снимок транзакции.
